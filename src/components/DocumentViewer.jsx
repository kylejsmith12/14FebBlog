...
const DocumentViewer = ({ title, selectedCategory, selectedView }) => {
        ...
        return (
          <>
            <DynamicTable
              ...
              variables={updatedDynamicTable} // Use updatedDynamicTable instead of state.variables.dynamicTable
            />
          </>
        );
      }
        ...
        <>
          ...
          {(selectedCategory === "harryPotter" || selectedView === "all") && (
            <p>
              Nearly ten years had passed since the{" "}
              <strong>{harryPotter["variable1"]}</strong> had woken up to find
              their nephew on the front step, but {harryPotter["variable2"]} had
              hardly changed at all. The sun rose on the same tidy front gardens
              and lit up the brass number four on the {harryPotter["variable1"]}
              ' front door; it crept into their living room, which was almost
              exactly the same as it had been on the night when Mr.{" "}
              {harryPotter["variable1"]} had seen that fateful news report about
              ...
              different-colored bonnets - but {harryPotter["variable3"]} was no
              longer a baby, and now the photographs showed a large blond boy
          ...
          {(selectedCategory === "lordOfTheRings" ||
            ...
            <p>
              ...
              merriment of <strong>{lordOfTheRings["variable1"]}</strong>.{" "}
              {lordOfTheRings["variable2"]} explains how folk get confused by
              her power. She sees into their hearts and makes them look at their
              mixed motives and ambitions, and some believe that she planted the
              dark motives there herself. (You can read more about this power{" "}
